...
#         default=False, description="Sort alphabetically the skills"
#     )
# ) -> JSONResponse:
...
@router.get(
    ...
    description="Send `ovos.api.skill_settings` message to the bus \
        and wait for `ovos.api.skill_settings.answer` response. This route \
        leverage the `skill-rest-api`",
...
async def retrieve_settings(
    skill_id: str = Path(
        ...
        example="skill-ovos-hello-world.openvoiceos",
    )
) -> JSONResponse:
