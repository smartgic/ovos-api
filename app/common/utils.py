...
def ws_send(payload: JSONStructure,
            wait_for_message: Optional[str] = None) -> JSONStructure:
    ...
    websocket = None
    try:
        websocket = create_connection(url=settings.ws_uri,
                                      timeout=settings.ws_conn_timeout)
    ...
    try:
        ...
        if wait_for_message:
            ...
            while time() < timeout_start + settings.ws_recv_timeout:
                ...
                if (
                    recv["type"] == wait_for_message and
                    not recv["context"]["authenticated"]
                   ):
                    data = recv
                    break
            websocket.close()
...
def requirements() -> bool:
    """Checks for requirements such as mycroft-api skill which will
    ...
    try:
        ...
        for key in skills["data"]:
            if (
                skills["data"][key]['id'] == constants.API_SKILL_ID and
                skills["data"][key]['active']
               ):
                status = True
        return status
