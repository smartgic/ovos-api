...
def sleep(confirm: Optional[bool] = True,
          dialog: Optional[Speak] = None) -> Speak:
    ...
    try:
        ...
        if requirements():
            sleep: JSONStructure = ws_send(
                payload, "mycroft.api.sleep.answer")
...
def wake_up(confirm: Optional[bool] = True,
            dialog: Optional[Speak] = None) -> Speak:
    ...
    try:
        ...
        wake_up: JSONStructure = ws_send(
            payload, "mycroft.api.wake_up.answer")
        if requirements():
    ...
    try:
        ...
        if requirements():
            info: JSONStructure = ws_send(
                payload, "mycroft.api.is_awake.answer")
            if info["context"]["authenticated"]:
                return info["data"]
            status_code = status.HTTP_401_UNAUTHORIZED
            msg = "unable to authenticate with mycroft-api skill"
